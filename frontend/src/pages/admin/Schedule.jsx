...
export default function Schedule() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setForm({ teacher: '', subject: '', school_class: '', day_of_week: 1, start_time: '08:00', end_time: '08:45' });
      loadAll();
    } catch (err) {
    ...
    <div className="flex-col gap-6 animate-fade-in" style={{ height: '100%' }}>
      ...
      <div className="flex-between">
        <div>
          ...
          <p className="text-muted">Haftalik dars jadvali — backend dan olingan</p>
        </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="glass" style={{ width: '100%', maxWidth: '480px', padding: '2rem', borderRadius: 'var(--radius-lg)' }}>
            ...
            <form onSubmit={handleCreate} className="flex-col gap-4">
              ...
              <div className="input-group">
                ...
                <select required className="input-field" value={form.school_class} onChange={e => setForm({ ...form, school_class: e.target.value })} style={{ background: 'var(--bg-darker)' }}>
                  <option value="">— Tanlang —</option>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div className="input-group">
                  <label style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.4rem' }}>Boshlanish</label>
                  <input type="time" required className="input-field" value={form.start_time} onChange={e => setForm({ ...form, start_time: e.target.value })} />
                </div>
                <div className="input-group">
                  <label style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.4rem' }}>Tugash</label>
